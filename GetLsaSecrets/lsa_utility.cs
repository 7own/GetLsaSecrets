...
namespace utilities
{
    class LSAUtility
    {
        ...
        public (string readable, string hexadecimal) GetSecret()
        {
            IntPtr LsaPolicyHandle = GetLsaPolicy(LSA_AccessPolicy.POLICY_GET_PRIVATE_INFORMATION);
            uint ntsResult = LsaRetrievePrivateData(LsaPolicyHandle, ref secretName, out IntPtr PrivateData);
            ReleaseLsaPolicy(LsaPolicyHandle);
            ...
            byte[] secretBytes = new byte[lusSecretData.Length];
            Marshal.Copy(lusSecretData.Buffer, secretBytes, 0, lusSecretData.Length);
            StringBuilder hexBuilder = new StringBuilder(secretBytes.Length * 2);
            ...
            {
                throw new Exception("Failed to decode data: " + e.Message);
            }
            finally
            {
            ...
            return (value,hexBuilder.ToString());
        }
